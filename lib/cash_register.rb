class CashRegister
  ...
  def initialize(employee_discount=20)
    @total = 0
    @employee_discount = employee_discount if employee_discount == 20
    @items = []
  end
  ...
  def apply_discount
    @total-(@total*(@employee_discount/100)) if @employee_discount == 20
    puts "There is no discount to apply." if @employee_discount == 0
  end
